...
const SplashScreens = () => {
    ...
    const fetchSplashScreens = async () => {
        try {
            setLoading(true);
            const res = await api.get('/splash');
            setSplashScreens(res.data.data.splashScreens || []);
        } catch (error) {
            toast.error('Failed to fetch splash screens');
        } finally {
            setLoading(false);
        }
    };
    ...
    const uploadImage = async () => {
        ...
        try {
            ...
            uploadFormData.append('splash_image', imageFile);

            const response = await api.post('/upload/splash', uploadFormData, {
                headers: {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (editingScreen) {
                await api.put(`/splash/${editingScreen.id}`, submitData);
                toast.success('Splash screen updated');
            } else {
                await api.post('/splash', submitData);
                toast.success('Splash screen created');
            }
    ...
    const toggleActive = async (id) => {
        try {
            await api.patch(`/splash/${id}/toggle`);
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900">Splash Screens</h1>
                    <p className="text-gray-500">Manage app loading screens</p>
                </div>
                <button onClick={openNew} className="btn btn-primary">
                    <PlusIcon className="w-5 h-5 mr-2" /> Add Splash Screen
                </button>
            </div>
                    ...
                    {splashScreens.map((screen) => (
                        <div key={screen.id} className="bg-white rounded-xl shadow-sm hover:shadow-lg transition-all border border-gray-100 overflow-hidden group">
                            <div className="relative">
                                ...
                                <div className="absolute bottom-3 left-3 bg-black/50 backdrop-blur-sm px-3 py-1 rounded-full text-white text-xs font-medium flex items-center">
                    ...
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden">
                        <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                            <h3 className="text-lg font-bold text-gray-900">
                                {editingScreen ? 'Edit Splash Screen' : 'Add Splash Screen'}
                            </h3>
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-4">
                            ...
                            <div className="border-2 border-dashed border-gray-300 rounded-xl p-4 hover:border-primary-400 transition-colors">
                                {imagePreview ? (
                                    ...
                                    <label className="flex flex-col items-center justify-center cursor-pointer py-12">
                                        ...
                                        <span className="text-sm text-gray-600 font-medium">Upload Splash Image</span>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    />
                                    <p className="text-xs text-gray-500 mt-1">How long to display (1000 = 1 second)</p>
                                </div>
                                <div>
